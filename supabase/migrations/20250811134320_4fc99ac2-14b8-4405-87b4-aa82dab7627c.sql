-- Update the conversation participant validation function to allow replies
      ...
      WHEN 'property_manager' THEN
        ...
        IF recipient_role = 'tenant' THEN
          IF NOT EXISTS (
            ...
            WHERE pma.manager_id = sender_id_param 
            AND t.user_id = recipient_id
          ) THEN
            RETURN FALSE;
          END IF;
        END IF;
      ...
      WHEN 'tenant' THEN
        ...
        ELSIF recipient_role = 'property_manager' THEN
          ...
          IF NOT EXISTS (
            ...
            WHERE t.user_id = sender_id_param 
            AND pma.manager_id = recipient_id
          ) THEN
            RETURN FALSE;
          END IF;
        ELSE
