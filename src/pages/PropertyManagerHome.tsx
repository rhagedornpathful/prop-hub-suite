...
const PropertyManagerHome = () => {
  ...
  const { schedule: weeklySchedule, loading, updateNextCheckDate } = usePropertyManagerSchedule(currentWeek);
  const { sendCheckReminder, notifyCheckCompleted } = usePropertyManagerNotifications();
  ...
  const startPropertyCheck = async (propertyId: string, propertyAddress: string) => {
    try {
  ...
  const completeCheck = async (propertyId: string, propertyAddress: string) => {
    try {
      await updateNextCheckDate(propertyId, new Date());
      await notifyCheckCompleted(propertyAddress, propertyId);
    } catch (error: any) {
      toast({
  ...
  return (
    <div className={`space-y-8 ${isMobile ? 'pb-20' : ''}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Property Schedule</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Scheduled property checks</p>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="flex gap-4">
          <Button onClick={() => navigate('/property-manager-properties')} variant="outline">
                      ...
                      return (
                        <div key={check.id} className="flex items-center justify-between p-4 border rounded-lg hover:shadow-sm transition-shadow">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button 
                              size="sm"
                              onClick={() => startPropertyCheck(check.id, check.property_address)}
                            >
