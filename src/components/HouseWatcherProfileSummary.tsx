...
import { useHouseWatchers } from '@/hooks/queries/useHouseWatchers';
import { format } from 'date-fns';

interface HouseWatcherProfileSummaryProps {
  showActions?: boolean;
  compact?: boolean;
}

export const HouseWatcherProfileSummary = ({ 
  showActions = true, 
  compact = false 
}: HouseWatcherProfileSummaryProps) => {
  const { user } = useAuth();
  const { data: profile } = useProfile();
  const { data: houseWatchers } = useHouseWatchers();

  const currentWatcher = houseWatchers?.find(hw => hw.user_id === user?.id);
  
  if (!currentWatcher || !profile) {
    return null;
  }
  ...
  const getInitials = () => {
    ...
    return `${firstName.charAt(0)}${lastName.charAt(0)}`.toUpperCase() || 'HW';
  };
  ...
  const performanceData = {
    completionRate: 92,
    propertiesManaged: currentWatcher.assigned_properties || 0,
    totalChecks: 156,
    onTimeRate: 94,
    clientRating: 4.8
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="secondary" className="text-xs">House Watcher</Badge>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <Badge variant="secondary">House Watcher</Badge>
                <Badge variant="outline">
                  Since {format(new Date(currentWatcher.created_at), 'MMM yyyy')}
                </Badge>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-green-600">{performanceData.onTimeRate}%</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-3 border rounded-lg">
              ...
              <div className="text-lg font-bold">{performanceData.propertiesManaged}</div>
